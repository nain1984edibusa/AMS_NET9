using Microservices.Demo.Policies.Service.Framework.Data;
using Microservices.Demo.Policies.Service.Framework.DI;
using Microservices.Demo.Policies.Service.Framework.Web;
using Microservices.Demo.Products.Service.Framework.DI;
using Microservices.Infrastructure.Trace;
using Scalar.AspNetCore;
using Steeltoe.Discovery.Client;

namespace Microservices.Demo.Policies.Service.Framework
{
    public static class HostExtensions
    {
        public static IServiceCollection AddHostServices(this IServiceCollection services, IConfigurationManager configuration)
        {
            services.AddDiscoveryClient(configuration);
            services.AddMessaging(configuration);
            services.AddRestClients(configuration);
            services.AddApplication();
            services.AddPersistence(configuration);
            services.AddMappings();
            services.AddHealthChecks(configuration);
            services.AddTelemetry();
            services.AddAllElasticApm();

            services.AddOpenApi();

            return services;
        }
        public static async Task<WebApplication> UseHostSetupAsync(this WebApplication app)
        {         
            app.MapOpenApi();
            app.MapScalarApiReference(options => { options.Servers = Array.Empty<ScalarServer>(); });
            app.MapAllEndpoints();
            await app.UseDbMessagingAsync();
            await app.SeedDatabaseAsync();
            app.MapHealthChecks();

            return app;
        }
    }
}
